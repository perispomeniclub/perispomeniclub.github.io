...
// 			self.height = map.getHeight();
		...
		// only for recognizing which layer it is, remove if done with debugging
	...
	getSprites: function(x1, y1, x2, y2){
		x1 = Math.floor(Math.max(Math.min(x1, this.getWidth()-1),0));
		y1 = Math.floor(Math.max(Math.min(y1, this.getHeight()-1),0));
		x2 = Math.ceil(Math.max(Math.min(x2, this.getWidth()-1),0));
		y2 = Math.ceil(Math.max(Math.min(y2, this.getHeight()-1),0));
	...
	// I should get rid of these functions
	getWidth: function(){
