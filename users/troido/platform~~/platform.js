...
// 	},
// 	getX: function(){
// 		return this.following.getX();
// 	},
// 	getY: function(){
// 		return this.following.getY();
// 	}
// 	
// });
...
var Platform = Class.extend({
	...
	loadMap: function(map){
		...
		map.layers.forEach(function(layer){
			var drawLayer;
			if (layer.type == "static" || layer.type == "background"){
				drawLayer = new StaticDrawLayer(layer.depth,layer.scale);
				/* This callback function looks complicated but that is because drawLayer.start should be executed
				 * when both the layer and the sprites are loaded
				 */
				...
				drawLayer = new DynamicDrawLayer(layer.depth, layer.scale);
				var game = new Game(layer.map, layer.blocks, function(){sprites.addLoadCallback(drawLayer.start.bind(drawLayer))});
	...
	draw: function(time){
		...
		if (camera){
			...
			this._drawLayers.forEach(function(layer){
				this._drawer.drawCanvas(layer.draw(this._drawer.getView()), layer.depth);
			}, this);
		}
		requestAnimationFrame(this.draw.bind(this));
	}
