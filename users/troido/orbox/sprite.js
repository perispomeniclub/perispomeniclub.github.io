...
define(["lib/inheritance"], function(Class){
	var Sprite = Class.extend({
		...
		height: 0,
		char: " ",
		init: function(img,sx,sy,swidth,sheight,cx,cy,width,height,char){
			this.img=img;
			this.sx=sx||0;
			this.sy=sy||0;
	...
	Sprite.loadSprites = function(spriteData){
		...
		for (var name in spriteData){
			...
			var spr = new Sprite(img, data.sx, data.sy, data.swidth, data.sheight, data.cx, data.cy, data.width, data.height, data.char);
			spr.name = name;
			sprites[name]=spr;
		}
		return sprites;
